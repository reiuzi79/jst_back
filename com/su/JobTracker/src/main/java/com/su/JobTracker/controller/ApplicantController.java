...
public class ApplicantController {
	...
	public ResponseEntity<?> getJobDetail(@RequestParam Integer job_posting_id){
		try {
			...
			result.setIsSaved(savedJobService.findIsSavedByUserIdAndJobPostingId(user_id, job_posting_id));
	...
	@DeleteMapping("/deleteSavedByPostingId")
	public ResponseEntity<?> deleteSavedByPostingId(@RequestParam Integer job_posting_id){
		try {
			var user_id = userService.getCurrentUser().getUserId();
			int count = savedJobService.deleteSavedJobByPostingId(user_id, job_posting_id);
			if(count == 1) {
				return ResponseEntity.ok(null);
			}else {
				return ResponseEntity.badRequest().body(null);
	...
	public ResponseEntity<?> modifyReview(@RequestBody ReviewRequest r){
		try {
			int user_id = userService.getCurrentUser().getUserId();
			var rating = r.getRating();
			var title = r.getTitle();
			var content = r.getContent();
			var review_id = r.getReview_id();
			var review = reviewService.findReviewById(review_id);
			if(review.getUserId() != user_id) {
				return ResponseEntity.status(StatusCode.UNAUTHORIZED.getCode()).body(null);
			}
